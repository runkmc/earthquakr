...
class QuakeListSpec : QuickSpec {
  override func spec() {
    ...
    let quake2 = Quake(magnitude: 4.55, locationString: "0km ESE of Concord, California", rawTime:date2, url:"http://earthquake.usgs.gov/earthquakes/eventpage/ak10992884", coordinates:[-121.958000,37.780000,12.39])
    let date3 = Int(xHoursAgo(3)(date: NSDate()).timeIntervalSince1970 * 1000)
    let quake3 = Quake(magnitude: 4.55, locationString: "0km ESE of Concord, California", rawTime:date3, url:"http://earthquake.usgs.gov/earthquakes/eventpage/ak10992884", coordinates:[-121,959000,37.780000,12.39])
    let date4 = Int(xHoursAgo(4)(date: NSDate()).timeIntervalSince1970 * 1000)
    let quake4 = Quake(magnitude: 4.55, locationString: "0km ESE of Concord, California", rawTime:date4, url:"http://earthquake.usgs.gov/earthquakes/eventpage/ak10992884", coordinates:[-121,957000,37.780000,12.39])
